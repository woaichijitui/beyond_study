...
type (
	...
	User struct {
		Id         uint64    `db:"id"`          // 涓婚敭ID
		Username   string    `db:"username"`    // 鐢ㄦ埛鍚
		Avatar     string    `db:"avatar"`      // 澶村儚
		Mobile     string    `db:"mobile"`      // 鎵嬫満鍙
		CreateTime time.Time `db:"create_time"` // 鍒涘缓鏃堕棿
		UpdateTime time.Time `db:"update_time"` // 鏈?悗淇?敼鏃堕棿
	}
)
